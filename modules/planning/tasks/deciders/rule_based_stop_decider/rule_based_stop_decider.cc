...
void RuleBasedStopDecider::CheckLaneChangeUrgency(Frame *const frame) {
  ...
  for (auto &reference_line_info : *frame->mutable_reference_line_info()) {
    ...
    //如果某参考线是变道，计算参考线是否阻塞，计算参考线代价值是否小于阈值,停止向下执行，继续下个循环
    ...
    if (frame->reference_line_info().size() <= 1 ||//如果参考线数量不大于1，意味着无换道场景
        (is_clear_to_change_lane_ && is_change_lane_planning_succeed_)) {//如果参考线没有被阻塞，且变道规划成功,停止向下计算
      continue;
    }
    ...
    if (reference_line->XYToSL(point, &sl_point) &&
      ...
      //计算最后一个点和车头的s距离，如果大于阈值，停止向下执行，继续循环 
...
bool RuleBasedStopDecider::CheckSidePassStop(
  ...
  PathData::PathPointType last_path_point_type =
      ...
      //遍历路径点集的属性，如果路径点集的属性从车道内转到反向车道，设置停止障碍，
...
//如果车辆车头与停车点的s轴距离>配置文件设置阈值，则返回false
