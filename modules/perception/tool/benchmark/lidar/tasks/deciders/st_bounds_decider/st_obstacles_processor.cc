...
Status STObstaclesProcessor::MapObstaclesToSTBoundaries(
  ...
  //遍历每个路径点，记录路径点为相邻车道的起点-终点vector：adc_low_road_right_segments_
  ...
  for (const auto* obs_item_ptr : path_decision->obstacles().Items()) {
    ...
    if (is_caution_obstacle) {//设置障碍物st图的结束时间
...
//计算障碍物的st边界，不同t时刻下障碍物与路径重合的路径索引
//st图生成，即障碍物与路径有重叠，则返回true
  ...
  *is_caution_obstacle = false;
  ...
  if (obs_trajectory.trajectory_point().empty()) {
    ...
    *obs_caution_end_t = planning_time_;
    ...
    //根据障碍物轨迹点查找障碍物与车辆重合的s索引并存放到lower_points和upper_points
...
//通过计算障碍物边角点和path点垂线方向的投影距离和车辆的车头、车尾关系来确定是否碰撞
bool STObstaclesProcessor::IsPathPointAwayFromObstacle(
  ...
  Vec2d path_pt(path_point.x(), path_point.y());//基础点
  Vec2d dir_pt(direction_point.x(), direction_point.y());//基础点相邻点
  LineSegment2d path_dir_lineseg(path_pt, dir_pt);//两点形成的线段
  LineSegment2d normal_line_seg(path_pt, path_dir_lineseg.rotate(M_PI_2));//起点为参考点，线段旋转90°
