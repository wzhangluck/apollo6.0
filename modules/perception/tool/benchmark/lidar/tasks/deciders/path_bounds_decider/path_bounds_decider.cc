...
//初始化，获取规划起始点相关参数，车道宽度，偏离道路中心线距离
void PathBoundsDecider::InitPathBoundsDecider(
  ...
  if (FLAGS_use_front_axe_center_in_path_planning) {
    planning_start_point =////若使用前轴定位，把后轴点坐标变换为前轴点坐标
  ...
  adc_l_to_lane_center_ = adc_frenet_l_ + offset_to_map;//adc偏离参考线值+参考线偏离中间车道值
...
bool PathBoundsDecider::GetBoundaryFromLanesAndADC(
  ...
  for (size_t i = 0; i < path_bound->size(); ++i) {
    ...
    double ADC_speed_buffer = (adc_frenet_ld_ > 0 ? 1.0 : -1.0) *
                              adc_frenet_ld_ * adc_frenet_ld_ /
                              kMaxLateralAccelerations / 2.0;
//如果是左侧借道，左侧边界需要加上左侧车道宽度
    double curr_left_bound_lane =//l值
        curr_lane_left_width + (lane_borrow_info == LaneBorrowInfo::LEFT_BORROW
                                    ? curr_neighbor_lane_width
                                    : 0.0);
//如果是右侧借道，右侧边界需要减去右侧车道宽度
    double curr_right_bound_lane =//l值
        -curr_lane_right_width -
        (lane_borrow_info == LaneBorrowInfo::RIGHT_BORROW
             ? curr_neighbor_lane_width
             : 0.0);
...
bool PathBoundsDecider::UpdatePathBoundaryWithBuffer(
  ...
  double new_l_min =//获取右侧边界，需要考虑车辆宽度，刨去
  ...
  double new_l_max = std::fmin(//获取左侧边界，需要考虑车辆宽度，刨去
...
//更新边界和中心线，中心线会对下一次下一个障碍物左饶右绕提供参考
...
//检测车道边界类型，若不借道，不进行检查并返回false
                 // 若左借道或右借道，检测边界类型若为实线，返回false
