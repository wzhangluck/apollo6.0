...
double LineSegment2d::GetPerpendicularFoot(const Vec2d &point,
  ...
  const double x0 = point.x() - start_.x();
  const double y0 = point.y() - start_.y();
  //目标点与起点组成的向量在线段方向投影，求得长度
  const double proj = x0 * unit_direction_.x() + y0 * unit_direction_.y();//内积
  *foot_point = start_ + unit_direction_ * proj;//求得垂足坐标
  return std::abs(x0 * unit_direction_.y() - y0 * unit_direction_.x());//外积，垂线长度
}
