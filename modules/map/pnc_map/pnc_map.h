...
class PncMap {
 ...
 private:
  ...
  struct RouteIndex {
    LaneSegment segment;
    std::array<int, 3> index;//依次为road、passage、lane的id
  };
  std::vector<RouteIndex> route_indices_;
  int range_start_ = 0;
  int range_end_ = 0;
  // routing ids in range
  std::unordered_set<std::string> range_lane_ids_;//从给定的route_indices_的索引开始往后的车道id
  std::unordered_set<std::string> all_lane_ids_;//route_indices_的所有车道id
